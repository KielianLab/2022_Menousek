...
files_to_read <- list.files(path = here('output', 'galea', 'GSEA', 'within cluster', 'C2'), pattern = '\\.tsv$', full.names = T)
...
files_to_read <- list.files(path = here('output', 'galea', 'GSEA', 'within cluster', 'C2'), pattern = '\\.tsv$', full.names = F)
files_to_read <- sub('\\.tsv', '', files_to_read)

for(i in 1:length(all_files)) {
  write.csv(all_files[i], here('output', 'galea', 'GSEA', 'within cluster', 'C2', 'converted', paste0(files_to_read[[i]], '.csv')))
}
